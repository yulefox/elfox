...
void Object::OnInit(void)
{
    ...
    if (itr != s_objs.end()) {
        ...
        LOG_ERROR("object", "(%d)<%lld:%lld> DUPLICATED initialization (%s) - (%s)",
                m_type,
                m_pid,
                m_id,
                obj->Name().c_str(),
...
void Object::IndexProto(pb_t *pb, oid_t xid, oid_t pid, int type, oid_t id, int idx)
{
    ...
    LOG_TRACE("object", "+   OBJ: %19lld %19lld %19lld %4d %6d", xid, pid, id, type, idx);
}
...
oid_t Object::GetContainer(oid_t pid, int type, bool add)
{
    ...
    if (cid < 0 && add) {
        cid = oid_gen();
        LOG_TRACE("object", "o CTNER: %19lld(%d)", cid, type);
...
void Object::DelContainer(oid_t id)
{
    ...
    LOG_TRACE("object", "- CTNER: %19lld(%3d)", id, s_containers.size());
}
...
void Object::DelChildren(oid_t pid, int type)
{
    ...
    if (type > 0) {
        ...
        E_DELETE(ism);
        s_containers.erase(itr);
        LOG_TRACE("object", "- CTNER: %19lld(%3d)", pid, s_containers.size());
    }
}
